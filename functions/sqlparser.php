...
//$pagedSql = "select pac.*,(select max(pacper_anio||'/'||pacper_mes) from cln_pac_per pp where pac.pac_id = pp.pac_id) as pac_ultper, os.os_nombre from cln_pac pac left join cln_eos os on pac.os_id = os.os_id where 1=1 and pac.pac_apellido like '%LOPEZ%' and pac.pac_nombre like '%LEONEL%'"; 
//$totalCountSql = SqlParser::ParseString( $pagedSql ) -> getCountQuery();
//echo $totalCountSql ;
    ...
    private function readsub($tokens, &$position) {
        ...
        while (!in_array($tokens[$position], self::$endparens) && $position < $tokenCount) {
            ...
            if (in_array($tokens[$position], self::$startparens)) {
                ...
                //$subs++; && bug en la version original 
            } else {
                $sub.= $tokens[$position];
            }
            $position++;
